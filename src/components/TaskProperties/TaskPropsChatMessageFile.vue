<template>
  <div>
    ...
    <div class="w-full inline-block">
      ...
      >
        <div class="mt-2 text-right font-medium mb-2 flex relative" v-if="!this.pics.includes(fileName.split('.').pop())">
          <div class="float-left"><FileMessage :file="file" /></div>
        <div class="float-right ml-[11.67px]">
          <span class="table font-bold text-[#4C4C4D] text-[13px] leading-[15px]">{{ fileName }}</span>
          <div
            v-if="time && size"
            class="group flex justify-between w-full text-[#7E7E80] dark:text-gray-300 "
          >
            <p class="text-[12px] leading-[15px] h-1[px font-normal">{{ size }}</p>
            <div
        ...
        <div class="mt-2 text-right text-[#7E7E80] font-medium mb-2 relative" v-else >
