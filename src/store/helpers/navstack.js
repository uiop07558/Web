...
export function checkTaskSource () {
  ...
  console.log('checking nav stack')
  ...
  if (lastNavStackElement && lastNavStackElement.key === 'taskListSource') {
    ...
    store.commit('basic', { key: lastNavStackElement.key, value: lastNavStackElement.value })
  } else {
    // If this is first sign in, create NavElement Today's tasks list because we got nothing in local store navstack item
    store.commit('basic', { key: 'taskListSource', value: { uid: '901841d9-0016-491d-ad66-8ee42d2b496b', param: new Date() } })
    store.commit(
...
export function checkGreedSource () {
  const navStack = computed(() => store.state.navbar.navStack)
  console.log('checking greed source')
  const lastNavStackElement = navStack.value[navStack.value.length - 1]
  if (lastNavStackElement && lastNavStackElement.key === 'greedSource') {
    if (lastNavStackElement.value.uid === '2bad1413-a373-4926-8a3c-58677a680714') {
      store.commit('basic', { key: 'mainSectionState', value: 'greed' })
      store.commit('basic', { key: 'greedPath', value: 'dashboard' })
    } else if (lastNavStackElement.value.uid === '2cf6b167-6506-4b05-bc34-70a8d88e3b25') {
      store.commit('basic', { key: 'mainSectionState', value: 'greed' })
      store.commit('basic', { key: 'greedPath', value: 'doitnow' })
    } else if (lastNavStackElement.value.uid === 'ed8039ae-f3de-4369-8f32-829d401056e9') {
      store.commit('basic', { key: 'mainSectionState', value: 'greed' })
      store.commit('basic', { key: 'greedPath', value: 'colors' })
    } else if (lastNavStackElement.value.uid === '00a5b3de-9474-404d-b3ba-83f488ac6d30') {
      store.commit('basic', { key: 'mainSectionState', value: 'greed' })
      store.commit('basic', { key: 'greedPath', value: 'tags' })
    } else if (lastNavStackElement.value.uid === '757be87d-c269-40e0-b224-6b2bb0e4f97d') {
      store.commit('basic', { key: 'mainSectionState', value: 'greed' })
      store.commit('basic', { key: 'greedPath', value: 'other' })
    } else {
      store.commit('basic', { key: 'mainSectionState', value: 'greed' })
      store.commit('basic', { key: 'greedPath', value: lastNavStackElement.greedPath })
    }

    const storeNavigator = computed(() => store.state.navigator.navigator)

    if (['new_private_projects', 'new_emps', 'new_delegate', 'new_private_boards'].includes(lastNavStackElement.greedPath)) {
      ...
      store.commit('basic', { key: lastNavStackElement.key, value: storeNavigator.value[lastNavStackElement.greedPath] })
    } else if (['tags_children', 'projects_children', 'boards_children'].includes(lastNavStackElement.greedPath)) {
      ...
      store.commit('basic', { key: 'taskListSource', value: { uid: lastNavStackElement.global_property_uid, param: lastNavStackElement.uid } })

      if (Array.isArray(storeNavigator.value[lastNavStackElement.greedPath])) {
        for (const elem of storeNavigator.value[lastNavStackElement.greedPath]) {
          makeSomethingMagicWithChildren(lastNavStackElement, elem.items)
        }
      } else {
        makeSomethingMagicWithChildren(lastNavStackElement, storeNavigator.value[lastNavStackElement.greedPath].items)
      }
    } else {
      // colors
      store.commit('basic', { key: lastNavStackElement.key, value: storeNavigator.value[lastNavStackElement.greedPath].items })
    }
  }
}
...
function makeSomethingMagicWithChildren (lastNavStackElement, items) {
  visitChildren(items, value => {
    if (value.uid === lastNavStackElement.uid) {
      store.commit('basic', { key: lastNavStackElement.key, value: value.children })
    }
  })
}
