...
export function getInspectorMessage (type, task) {
  ...
  switch (type) {
    ...
    case IGNORE_MESSAGE_TYPE: return performerName + ', вы не ответили после того как получили задачу от ' + performerName + '. Пожалуйста напишите вопросы если есть' + (currentUser === task.uid_customer ? '' : ', или нажмите: ')
    ...
    case PERFOMER_DOESNT_HAVE_PHONE_NUMBER_MESSAGE_TYPE: return 'Я уже хотел связаться с ' + performerName + ', но в базе нет номера телефона исполнителя или что-то не так с данными timezone. Пожалуйста, добавьте номер телефона исполнителя.'
    case SMS_MESSAGE_TYPE: return 'Я отправил смс исполнителю на номер ' + performerPhone + '.'
    case BEFORE_DEADLINE_MESSAGE_TYPE: return performerName + ', вы завершили задачу в срок и получаете +15 баллов к вашей карме.'
    case AFTER_DEADLINE_MESSAGE_TYPE: return performerName + ', вы просрочили задачу и ничего не написали даже после дедлайна, пожалуйста позвоните руководителю и обьясните причину. С вас снимается 15 баллов за такое грубое нарушение.'
  }
}
